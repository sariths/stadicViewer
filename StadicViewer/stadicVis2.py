...
class Main(QtGui.QDialog,Ui_Form):
    def __init__(self,parent=None,jsonFile=None,spaceID=None):
        ...
        self.txtSpaceColorsMax.setText(str(self.illuminanceMaxValDefault))
        self.txtSpaceColorsMin.setText(str(self.illuminanceMinValDefault))
    ...
    def setColorSettings(self):
        if self.currentPlotIsIlluminance:
            try:
                self.illuminanceUpperMaskValue = float(str(self.txtSpaceColorsUpperMask.text()))
            except ValueError:
                pass
            try:
                self.illuminanceLowerMaskValue = float(self.txtSpaceColorsLowerMask.text())
            except ValueError:
                pass
            self.illuminanceMaxVal = float(self.txtSpaceColorsMax.text())
            self.illuminanceMinVal = float(self.txtSpaceColorsMin.text())
            self.plotIlluminance()

        else:
            try:
                self.metricsUpperMaskValue = float(str(self.txtSpaceColorsUpperMask.text()))
            except ValueError:
                pass
            try:
                self.metricsLowerMaskValue = float(self.txtSpaceColorsLowerMask.text())
            except ValueError:
                pass
            self.metricsMaxVal = float(self.txtSpaceColorsMax.text())
            self.metricsMinVal = float(self.txtSpaceColorsMin.text())
            self.plotMetrics()
    ...
    def resetColorSettings(self):
        ...
        if self.currentPlotIsIlluminance:
            ...
            self.txtSpaceColorsMax.setText(str(self.metricsMaxValDefault))
            self.txtSpaceColorsMin.setText(str (self.metricsMinValDefault))
            self.metricsLowerMaskColor = None
            self.metricsUpperMaskColor = None
    ...
    def plotIlluminance(self):
        if not self.illuminanceActivated:
            ...
            self.layoutSpace.addWidget(self.toolbar)
            self.layoutSpace.addWidget(self.canvas)
...
class Main2(Main):
    def __init__(self,parent=None,jsonFile=None,spaceID=None):
        super(Main2,self).__init__(parent,jsonFile,spaceID)
...
def main(jsonFile=None,spaceID=None,*args):
    ...
    form = Main2(jsonFile=jsonFile,spaceID=spaceID)
    form.show()
    app.exec_()
