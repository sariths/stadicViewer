# coding=utf-8
"""
    Contains a class to process readStadicData files.
"""

from __future__ import print_function

from parseJson import StadicProject
...
class VisData(object):
    def __init__(self,fileName=None,spaceIndex=0):
        ...
        self.dataAllFiles = {}
        """List of all the possible files as per the naming scheme."""

        self.dataAllFilesAvailable = {}
        """List of only those files that are physically present in disc."""

        self.dataDayIllFilesList = []
        """FileKey,FileName tuples for all the daylight ill files. The main ill file will be on the top of this list."""

        self.dataMetricsFilesList = []
        """FileKey,FileName tuples for all the metrics files."""
        self.dataSignalsFilesList = []
        """FileKey,FileName tuples for all the signals files."""
        self.dataScheduleFilesList = []
        """FileKey,FileName tuples for all the schedules files."""
        self.dataSettingsFilesList = []
        """FileKey,FileName tuples for all the settings files."""

        self.dataTimeSeriesLists =[]
        """FileKey,FileName tuples for all the time series files with 8760 values.."""
...
if __name__ == "__main__":
    ...
    file1= r"C:\example1\Mistrick_Stadic_Example.json"
    vis = VisData(fileName=file1)
    print(vis.dataAllFiles)
    print(vis.dataAllFilesAvailable)